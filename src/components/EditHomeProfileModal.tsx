...
interface EditHomeProfileModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const EditHomeProfileModal: React.FC<EditHomeProfileModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  homeProfile
  ...
  // Initialize form data when modal opens or homeProfile changes
  useEffect(() => {
    if (isOpen && homeProfile) {
      setFormData({
        address: homeProfile.address || '',
        yearBuilt: homeProfile.year_built || new Date().getFullYear(),
        squareFootage: homeProfile.square_footage || 0,
        bedrooms: homeProfile.bedrooms || 1,
        bathrooms: homeProfile.bathrooms || 1,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get the profile ID from the homeProfile
      const profileId = parseInt(homeProfile.id || '0');
      if (!profileId) {
        throw new Error('Invalid profile ID');
      }

      const updatedProfile = await HomeProfileService.updateProfile(profileId, profileData);
      ...
      setSubmitError(
        ...
        'Failed to update home profile. Please try again.'
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Edit Home Profile</h2>
          </div>
          <button
            onClick={handleClose}
            disabled={isLoading}
